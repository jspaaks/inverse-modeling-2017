function OUT = eastermodel(parVec,Extra)
% easter island 
...
ResourceMeas = Extra.ResourceMeas;
...
OUT = PopSim(ismember(tSim,tMeas));
...
if ~Extra.dontPlot
    ...
    if mod(floor(c(6)),plotInterval)==0
        ...
        hold on
        hResSim = plot(tSim,ResourceSim,'linestyle','-','color',[0,0.5,0]);
        set(gca,'xlim',[tSim(1),tSim(end)],'ylim',[0,250])
        ...
        set(gca,'xlim',[tSim(1),tSim(end)],'ylim',[0,15000])        
        xlabel('Time [years]')
        ylabel('Population')
