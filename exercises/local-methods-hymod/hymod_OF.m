function [OF] = hymod_OF(Pars,Extra); 
% Runs the HYMOD model

% Define the rainfall
PET = Extra.PET; 
Precip = Extra.Precip;
MeasData=Extra.MeasData(Extra.calPeriod(1):Extra.calPeriod(2));
...
x_quick(1:3,1) = 0; 
wu = 65;
t=Extra.calPeriod(1)-wu;   % 65-day warm up
MaxT = Extra.calPeriod(end); 
outflow = [];
...
while t<MaxT+1,
   ...
   output(t-Extra.calPeriod(1)+1+wu,1) = (QS + outflow)*22.5;
   t = t+1;   
end;
SimRR = output(wu+1:end,1);

OF=sum((SimRR-MeasData).^2);
