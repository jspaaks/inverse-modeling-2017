...
% TODO switch to using \texttt{linregr} as preparation for DiffEvoMetro below.
...
\smallqo{Use the information from \texttt{evalResults} to re-run the last
\texttt{nSets = 500} samples, but make sure to store the predictions associated
with each parameter set as a row in a larger matrix \texttt{ySims} of size
\texttt{nSets x nObs}.}

\smallqo{Use the \texttt{prctile} function to calculate the 95\% confidence
interval and the median at each time step using
\texttt{prctile(ySims,[2.5,50,97.5])}. Visualize the \texttt{prctile} results
together with the observations \texttt{yObs}, and the \texttt{ySim} associated
with the best-scoring parameter set.}

% TODO Your result should look more or less like Fig.~\ref{fig:diffevo-metro-result}.
 
\smallqo{You could also construct the 2.5\% line by calculating the 2.5\%
