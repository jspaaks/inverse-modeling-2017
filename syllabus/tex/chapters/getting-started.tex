...
\smallq{Change Octave's working directory to
`./exercises/differential-evolution-metropolis/' and use your editor to open
`runDiffEvoSO.m'. This is more or less the same script that we were using as a
starting point for the multi-objective optimization.}

\smallq{Save `runDiffEvoSO.m' as `runDiffEvoMetro.m'. You will be making your
edits in the latter script.}
...
\smallq{Adapt the acceptance scheme in `runDiffEvoMetro.m' in accordance with
...
\smallq{Use the \texttt{prctile} function to construct a plot that shows the
measured discharge, the best simulated discharge, together with the median and
95\% confidence interval of parameter uncertainty. Your result should look more
or less like Fig.~\ref{fig:diffevo-metro-result}.}
...
\smallq{You could also construct the 2.5\% line by calculating the 2.5\%
quantile based on the last \texttt{nSets} parameter sets in \texttt{evalResults}
(as opposed to the predictions associated with those sets) and run the model
with that. Why is it generally incorrect to construct parameter intervals in
this fashion?}
